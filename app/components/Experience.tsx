        ...
        <div className="md:w-2/3 space-y-16">
          {experiences.map((exp, index) => (
            <div key={index} className="relative pl-8 border-l border-primary/20">
              {/* Timeline dot */}
              <div className="absolute top-0 -left-1.5 w-3 h-3 rounded-full bg-primary border-4 border-background" />
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2 pt-4">
                  {exp.techStack.map((tech, i) => (
                    <span 
                      key={i} 
                      className="px-3 py-1 bg-secondary text-secondary-foreground text-xs font-bold rounded-md hover:bg-primary/20 hover:text-primary transition-colors cursor-default"
                    >
                      {tech}
                    </span>
                  ))}
